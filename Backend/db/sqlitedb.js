...
db.serialize(() => {
    ...
    exports.createHomework = function createHomework(req, callback) {
        ...
        db.get("INSERT INTO homework (fach, name, description, date, author) VALUES (?, ?, ?, ?, ?)", fach, name, description, Date1, author , (err, row) => {
            if (err) {
                console.error(err);
                callback(err,false);
            } else {
    ...
    exports.checkHomework = function deleteHomework(req, callback) {
        ...
        db.get("UPDATE homework SET fertig = ? WHERE id = ?", done, id, (err, row) => {
            if (err) {
                console.error(err.message);
                callback(err,false);
            } else {
